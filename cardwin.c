...
void YesNo(char *szMess, void(*Yesfunc), void(*Nofunc), int deflt)
{
	...
	gtk_signal_connect(GTK_OBJECT(but2), "clicked", GTK_SIGNAL_FUNC(Nofunc),
			   dialog_window);
...
void ShowMessage(char *szTitle, char *szMess)
{
	...
	dialog_window = gtk_dialog_new();
	gtk_signal_connect(GTK_OBJECT(dialog_window), "destroy",
			   GTK_SIGNAL_FUNC(CloseDialog), NULL);
	...
	gtk_signal_connect(GTK_OBJECT(button), "clicked",
			   GTK_SIGNAL_FUNC(CloseShowMessage), dialog_window);
...
void AddCard(GtkWidget *widget, gpointer data) // data is *listbox
{
	...
	gtk_signal_connect(GTK_OBJECT(butOK), "clicked",
			   GTK_SIGNAL_FUNC(CloseandAdd), d_window);
	gtk_signal_connect(GTK_OBJECT(butCancel), "clicked",
			   GTK_SIGNAL_FUNC(CloseShowMessage), d_window);
	gtk_signal_connect(GTK_OBJECT(ndex), "activate",
			   GTK_SIGNAL_FUNC(CloseandAdd), d_window);
	...
	gtk_box_pack_start(GTK_BOX(GTK_DIALOG(d_window)->vbox), label, TRUE,
			   TRUE, 0);
	gtk_box_pack_start(GTK_BOX(GTK_DIALOG(d_window)->vbox), ndex, TRUE,
			   TRUE, 0);
...
void CloseandAdd(GtkWidget *widget, gpointer *data)
{
	...
	if (strlen(entry) != 0) {
		rindx = g_list_insert_sorted(rindx, (gchar *)buffnew,
					     (GCompareFunc)eval_items);
...
void DelCard(GtkWidget *widget, gpointer data) // data: "Delete"
{
	...
	buffnew = g_strdup_printf("%s: Remove this card?\n%s",
				  (gchar *)g_list_nth_data(rindx, count),
				  (gchar *)g_list_nth_data(rdata, count));
...
void men_move(gpointer data, guint action, GtkWidget *widget)
{
	...
	case 0:
		f_op = 0;
		gtk_file_selection_set_filename(GTK_FILE_SELECTION(f_sel),
						"./*.crd");
		f_openh(f_sel);
		break;
	case 1: // new
	...
	case 15: // about
		ShowMessage(
			"Mine!",
			"Windows Cardfile Viewer for Linux:\nWorking Version Gtk 0.09\nby C. Menard.");
		break;
...
int main(int argc, char *argv[])
{
	...
	gtk_text_view_set_editable(GTK_TEXT_VIEW(card_data), TRUE);
	gtk_text_view_set_wrap_mode(GTK_TEXT_VIEW(card_data), GTK_WRAP_WORD);
	...
	//	gtk_file_selection_set_filename(GTK_FILE_SELECTION(f_sel), "./*.crd");
	gtk_signal_connect(GTK_OBJECT(f_sel), "delete_event",
			   GTK_SIGNAL_FUNC(hide_win), NULL);
	gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(f_sel)->ok_button),
			   "clicked", file_back, NULL);
	gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(f_sel)->cancel_button),
			   "clicked", GTK_SIGNAL_FUNC(hide_win), NULL);
	...
	gtk_signal_connect(GTK_OBJECT(butP), "clicked",
			   GTK_SIGNAL_FUNC(callback), (gpointer)listbox);
	gtk_signal_connect(GTK_OBJECT(butN), "clicked",
			   GTK_SIGNAL_FUNC(callnext), (gpointer)listbox);
	gtk_signal_connect(GTK_OBJECT(butA), "clicked",
			   GTK_SIGNAL_FUNC(AddCard), (gpointer)listbox);
	gtk_signal_connect(GTK_OBJECT(butD), "clicked",
			   GTK_SIGNAL_FUNC(DelCard), (gpointer) "Delete");
	gtk_signal_connect(GTK_OBJECT(butQ), "clicked",
			   GTK_SIGNAL_FUNC(exitapp), NULL);
	gtk_signal_connect(GTK_OBJECT(listbox), "select_child",
			   GTK_SIGNAL_FUNC(l_select), NULL);
...
void l_select(GtkWidget *widget, guint ndata) // widget is callback's listbox
{
	...
	GList *node;
	GtkWidget *ditem;
	GtkTextBuffer *buf = gtk_text_view_get_buffer(GTK_TEXT_VIEW(card_data));
	if (!first) {
		...
		if (g_strcasecmp((gchar *)entry, (gchar *)node->data) != 0) {
			node->data = (gpointer)
				entry; // on removing old data: memory leak?
			altered = TRUE;
			g_print("%s\n", entry); // buffnew
		} else
	...
	gtk_text_buffer_set_text(buf, ptr1, strlen(ptr1));
	first = FALSE;
	currstat();
}
...
void currstat()
{
	...
	gtk_editable_insert_text(GTK_EDITABLE(statdata), head, strlen(head),
				 &position);
	ptr1 = g_list_nth_data(rindx, 0);
	ptr2 = g_list_nth_data(rdata, 0);
	head = g_strdup_printf("\n\nMGC length: %d   End:  %d (cards): %d",
			       indpos, datpos, (guint16)ptr[3]);
	gtk_editable_insert_text(GTK_EDITABLE(statdata), head, strlen(head),
				 &position);
	head = g_strdup_printf(
